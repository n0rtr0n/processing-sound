...
public class OPC implements Runnable
{
    ...
    void ledRing(int index, int count, float x, float y, float radius, float angle)
    {
        for (int i = 0; i < count; i++) {
            ...
            led(index + i, (int)(x - radius * Math.cos(a) + 0.5),
                    (int)(y - radius * Math.sin(a) + 0.5));
        }
    }
                 ...
                 boolean flip)
    {
        ...
        for (int i = 0; i < numStrips; i++) {
            ledStrip(index + stripLength * i,
                    stripLength,
                    (float) (x + (i - (numStrips-1)/2.0) * stripSpacing * c),
                    (float) (y + (i - (numStrips-1)/2.0) * stripSpacing * s),
                    ledSpacing,
                    angle,
                    zigzag && ((i % 2) == 1) != flip
            );
    ...
    public void draw()
    {
        ...
        for (int i = 0; i < numPixels; i++) {
            ...
            int pixel = this.parent.pixels[pixelLocation];
            ...
            if (enableShowLocations) {
                this.parent.pixels[pixelLocation] = 0xFFFFFF ^ pixel;
            }
        }
